...
서브포켓의 API Key 권한을 사용하여 해당 서브포켓에서 발생한 포켓 간 자산 이전 내역 목록을 조회하는 API입니다.

### 서브포켓 자산 이전 목록 조회 가능 범위

본 API는 **서브포켓의 API Key 권한**을 사용하여 해당 서브포켓에서 발생한 포켓 간 자산 이전 내역을 조회하는 API입니다.<br />조회 결과에는 해당 서브포켓을 기준으로 발생한 이전 내역만 포함합니다.<br />조회하고자하는 포켓 UUID는 <Anchor target="_blank" href="ref:list-pockets">포켓 정보 조회 API</Anchor>를 통해 확인할 수 있습니다.
...
조회 결과에는 해당 API Key가 속한 서브포켓과 관련된 이전 내역만 포함됩니다.

* `direction = in` : 해당 서브포켓으로 들어온 이전 내역만 조회합니다.
* `direction = out` : 해당 서브포켓에서 나간 이전 내역만 조회합니다.
* `direction = all` : 해당 서브포켓과 관련된 모든 이전 내역만을 조회합니다.
...
서브포켓 키발급 페이지에서 \[자산이전] 권한이 설정된 API Key를 사용해야 합니다. <br />
  ...
  "paths": {
    "/v1/pockets/transfers": {
      "get": {
        ...
        "summary": "서브포켓 자산 이전 목록 조회",
        "description": "서브포켓의 API Key 권한을 사용하여 해당 서브포켓에서 발생한 포켓 간 자산 이전 내역 목록을 조회하는 API입니다.",
        "tags": [
          "Exchange"
        ],
        "x-readme": {
        ...
        "parameters": [
          {
            ...
            "required": false,
            "schema": {
              "$ref": "#/components/schemas/PocketTransferDirection"
            },
            "allowReserved": true,
            "examples": {
              "default": {
        ...
        "responses": {
          "200": {
            "description": "List of pocket transfers",
            "content": {
              "application/json": {
                "schema": {
                  "type": "array",
